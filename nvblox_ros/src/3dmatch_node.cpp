...
Nvblox3DMatchNode::Nvblox3DMatchNode(ros::NodeHandle& nodeHandle)
    : nodeHandle3D_(nodeHandle) {
  setupRos();
}

void Nvblox3DMatchNode::setupRos() {
  double time_between_frames = 0.5;  // seconds
  // time_between_frames =declare_parameter<float>("time_between_frames",
  // time_between_frames);

  nodeHandle3D_.param<double>(
  ...
  mesh_publisher_ =
      nodeHandle3D_.advertise<nvblox_msgs::Mesh>("mesh", 1, false);

  // Image settings
  // rotate_optical_frame_ =     declare_parameter<bool>("rotate_optical_frame",
  // rotate_optical_frame_); rotate_world_frame_ =
  // declare_parameter<bool>("rotate_world_frame", rotate_world_frame_);

  nodeHandle3D_.param<bool>(
      nodeHandle3D_.getNamespace() + "/rotate_optical_frame",
      rotate_optical_frame_, false);
  nodeHandle3D_.param<bool>(
      nodeHandle3D_.getNamespace() + "/rotate_world_frame", rotate_world_frame_,
      true);

  // Create the layers.
  // voxel_size_ = declare_parameter<float>("voxel_size", voxel_size_);

  nodeHandle3D_.param<float>(nodeHandle3D_.getNamespace() + "/voxel_size",
                             voxel_size_, 0.2);
...
bool Nvblox3DMatchNode::integrateFrame(const int frame_number) {
  if (!tsdf_layer_) {
    ROS_ERROR("No layer created. Please create a layer first.");
    return false;
  }
  ...
  if (!datasets::threedmatch::internal::parseCameraFromFile(
  ...
  if (!datasets::load16BitDepthImage(
          datasets::threedmatch::internal::getPathForDepthImage(
              base_path_, sequence_num_, frame_number),
          &depth_image)) {
    return false;
  }
  // Get the transform.
  ...
  tsdf_integrator_.integrateFrame(depth_image, T_L_C, camera, tsdf_layer_.get(),
                                  &updated_blocks);
  // Mesh integrator
  mesh_integrator_.integrateBlocksGPU(*tsdf_layer_, updated_blocks,
                                      mesh_layer_.get());
